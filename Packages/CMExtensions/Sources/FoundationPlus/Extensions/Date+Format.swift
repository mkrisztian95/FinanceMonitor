...
public extension Date {
    var userFriendlyFormatted: String {
    ...
    func isSameDay(_ comparisonDate: Date, timeZone: TimeZone = .current) -> Bool {
        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = timeZone
    ...
    var startOfDay: Date {
        calendar(timeZone: TimeZone(identifier: "UTC")!).startOfDay(for: self)
    }
    ...
    func toString(
        ...
        let formatter = Date.cachedFormatter(
            dateStyle,
            timeStyle: timeStyle,
    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        let hours: Double = round(min / 60)
        let days: Double = round(hours / 24)

        if sec < 60 {
            if sec < 10 {
                if isPast {
                    return strings?[.nowPast] ?? NSLocalizedString(
                        "Только что",
                        comment: "Date format"
                    )
                } else {
                    return strings?[.nowFuture] ?? NSLocalizedString(
                ...
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString(
                        "%.f секунд назад",
                        comment: "Date format"
                    )
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString(
                        "в течении %.f секунд",
                        comment: "Date format"
                    )
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    return strings?[.oneMinutePast] ?? NSLocalizedString(
                        "минуту назад",
                        comment: "Date format"
                    )
                } else {
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString(
                        "в течении %.f минуты",
                        comment: "Date format"
                    )
                }
                ...
                let string: String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString(
                        "%.f минут назад",
                        comment: "Date format"
                    )
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString(
                        "в течении %.f минут",
                        comment: "Date format"
                    )
                }
        ...
        if hours < 24 {
            if hours == 1 {
                if isPast {
                    return strings?[.oneHourPast] ?? NSLocalizedString(
                        "час назад",
                        comment: "Date format"
                    )
                } else {
                    return strings?[.oneHourFuture] ?? NSLocalizedString(
                        "в течении часа",
                        comment: "Date format"
                    )
                }
                ...
                let string: String
                if isPast {
                    string = strings?[.hoursPast] ?? NSLocalizedString(
                        "%.f часов назад",
                        comment: "Date format"
                    )
                } else {
                    string = strings?[.hoursFuture] ?? NSLocalizedString(
                        "в течении %.f часов",
                        comment: "Date format"
                    )
                }
                return String(format: string, hours)
            }
        }
        if days < 7 {
            if days == 1 {
                if isPast {
                    return strings?[.oneDayPast] ?? NSLocalizedString(
                        "вчера", comment:
                            "Date format"
                    )
                } else {
                    return strings?[.oneDayFuture] ?? NSLocalizedString(
                        "завтра",
                        comment: "Date format"
                    )
                }
                ...
                let string: String
                if isPast {
                    string = strings?[.daysPast] ?? NSLocalizedString(
                        "%.f дней назад",
                        comment: "Date format"
                    )
                } else {
                    string = strings?[.daysFuture] ?? NSLocalizedString(
                        "в течении %.f дней",
                        comment: "Date format"
                    )
                }
                return String(format: string, days)
            }
        }
    ...
    static func components(_ fromDate: Date, timeZone: TimeZone = .current) -> DateComponents {
        var calendar = Calendar(identifier: .gregorian)
        calendar.timeZone = timeZone
    ...
    private static func cachedFormatter(
        ...
        let hashKey = "\(dateStyle.hashValue)" +
        "\(timeStyle.hashValue)" +
